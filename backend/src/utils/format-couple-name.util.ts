 ...
 * 3. Only partner 2 present -> "Nisansala" (or "Nisansala Perera" if last name exists)
 * 4. Neither present or placeholder "Visitor" -> fallback (default "Couple")
 */
export function formatCoupleName(
  ...
  // 1. If both partners' first names are provided
  if (p1 && p2) {
    return `${p1} & ${p2}`;
  }

  // 2. If only partner 1 is provided
  if (p1) {
    const l1 = (visitor.visitor_lname || '').trim();
    return l1 ? `${p1} ${l1}` : p1;
  }

  // 3. If only partner 2 is provided
  if (p2) {
